...
export const loadIngredientsSuccess = (ingredients: IDrgagItem[]): ILoadIngredientsSuccess => ({type: LOAD_INGREDIENTS_SUCCESS, ingredients});
export const loadIngredientsError = (isError: boolean): ILoadIngredientsError => ({type: LOAD_INGREDIENTS_ERROR, isError});
export const updateIngredients = (ingredients: IDrgagItem[]): IUpdateIngredients => ({type: UPDATE_INGREDIENTS, ingredients});

export const setConstructorIngredients = (ingredient: IDrgagItem): ISetConstructorIngredients => ({type: SET_CONSTRUCTOR_INGREDIENTS, ingredient});
export const updateConstructorIngredients = (ingredients: IDrgagItem[]): IUpdateConstructorIngredients => ({type: UPDATE_CONSTRUCTOR_INGREDIENTS, ingredients});
export const delConstructorIngredients = (curentIndex: number): IDelConstructorIngredients => ({type: DEL_CONSTRUCTOR_INGREDIENTS, curentIndex});
export const setConstructorBun = (constructorBun: IIngredientItem | null): ISetConstructorBun => ({type: SET_CONSTRUCTOR_BUN, constructorBun});
